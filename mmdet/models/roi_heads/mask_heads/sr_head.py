...
class SRHead(BaseModule):
    ...
    def __init__(
        ...
        if ignore_label:
            loss_sr['ignore_index'] = ignore_label
        if loss_weight:
            loss_sr['loss_weight'] = loss_weight
    ...
    def loss(self, mask_preds: Tensor, labels: Tensor) -> Tensor:
        ...
        loss_sr = self.criterion(mask_preds, labels)
        return loss_sr
